...
local function filename()
  return f(function(_args, snip)
    local full_path = snip.snippet.env.TM_FILEPATH
    local project_path = vim.split(full_path, "/val-doc-storage/", true)[2]
    local module_path = vim.split(project_path, ".", true)[1]
    module_path = vim.split(module_path, "/", true)

    local modules_and_class = {}

    for index, path in ipairs(module_path) do
      if index > 1 then
        table.insert(modules_and_class, path)
      end
    end

    return modules_and_class
  end)
end
...
ls.add_snippets("ruby", {
  ...
  s("rspec", {
    -- Frozen string and rails_helper with empty lines
    t({ "# frozen_string_literal: true", "", "require 'rails_helper'", ""}),
